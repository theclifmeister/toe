...
[bar]
# Omarchy's bar, across the top of every display in place of the macOS menu bar: the menu and
# the workspaces on the left, the clock in the centre, and bluetooth, network, audio, displays
# and power on the right, each a Nerd Font glyph that opens the matching Settings pane. While
# this is on the system's menu bar is set to hide itself — it still slides in when the pointer
# reaches the top edge, over the bar, which is how you reach an application's menus — and the
# tiles start under the bar. SUPER+SHIFT+SPACE hides the bar and gives the tiles its strip back.
# The setting outlives toe: it is journalled before it is changed and put back at quit, and at
# the next launch after an unclean exit. Off, the workspace strip is a menu bar item, as before.
enabled = true
...
# tiles take its strip back; `bar show` and `bar hide` are the one-way spellings.
